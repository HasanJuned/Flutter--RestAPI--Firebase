...
class CartController extends GetxController {
  ...
  bool _getCartToListInProgress = false;
  ...
  bool get getCartToListInProgress => _getCartToListInProgress;

  CartListModel get catListModel => _cartListModel;
  ...
  Future<bool> getCartList() async {
    _getCartToListInProgress = true;
    update();

    final response = await NetworkCaller.postRequest(url: '/CartList');
    _getCartToListInProgress = false;
    if (response.isSuccess) {
  ...
  void updateTotalPrice() {
    ...
    for (var cart in _cartListModel.data!) {
      _totalPrice += (double.tryParse(cart.product?.price ?? '0') ?? 0) * cart.count;
    }
    update();
  }
}
